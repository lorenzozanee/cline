...
- `run_commands` no longer hands `child_process.spawn` a bare program name on Windows. libuv resolves a bare name by searching the command's working directory before PATH, which PowerShell itself never does, so a `powershell.exe` planted in a workspace could shadow the system shell — for the CLI's bare `powershell` default shell on every command, for structured `{ command, args }` input, and for the nested-wrapper unwrapping below. Program names are now resolved through PATH before spawning and a name that is not on PATH fails like the missing program it is; a same-edition nested wrapper keeps the host's configured shell path instead of the bare name it was written with; and the executor's own `taskkill.exe` and `powershell.exe` probe use their fixed `System32` paths
